...
class col_new(Module):
    ...
    def p_losses(self, x_start, t, noise = None, offset_noise_strength = None):
        ...
        x = self.q_sample(x_start = x_start[:,0:c,...], t = t, noise = noise)
...
class MyCOL(nn.Module):
    ...
    #        num_steps = t[batch_index].item()  # Extract the scalar value
    #        for ti in reversed(range(num_steps)):
    #            # Convert ti to a tensor to maintain consistency with network input
    #            ti_tensor = torch.tensor(ti, device=x.device, dtype=torch.long)
    #            x[batch_index:batch_index+1] = self.network(x[batch_index:batch_index+1], ti_tensor)
    #    return x
...
'''
def OT_col(x, avg_window=10, tol=1e-6, MinIter=10, MaxIter=100):
      tries = x.shape[1]
      sum_ = 1000000
      dists_coll = torch.zeros(MaxIter+1, device=x.device)
      dists_coll[0] = torch.mean( torch.sum( (x[0,...] - x[1,...])**2, axis=-1) )
      for nt in range(MaxIter):
        iss = torch.randperm(tries, device=x.device)
        i1s = iss[:int(tries/2)]
        i2s = iss[int(tries/2):]

        # Calculate the initial distances
        s0 = torch.sum( (x[0,i1s] - x[1,i1s])**2, axis=-1) + torch.sum( (x[0,i2s] - x[1,i2s])**2, axis=-1)

        # Calculate the distances after the swap
        s1 = torch.sum( (x[0,i1s] - x[1,i2s])**2, axis=-1) + torch.sum( (x[0,i2s] - x[1,i1s])**2, axis=-1)
        ...
        # Use in-place swap to ensure y remains on the correct device
        y_clone = x[1,...].clone()  # Workaround for in-place swapping with PyTorch
        y_clone[accepted_i1s], y_clone[accepted_i2s] = y_clone[accepted_i2s], y_clone[accepted_i1s]
        x[1,...] = y_clone  # Update y after swapping
        torch.cuda.empty_cache()

        dists_coll[nt+1] = torch.mean( torch.sum( (x[0,...] - x[1,...])**2, axis=-1) )
        if nt>avg_window and nt > MinIter:
          sum_0 = torch.sum(dists_coll[-avg_window:])
          if abs(sum_ - sum_0)/sum_0 < tol:
            break
